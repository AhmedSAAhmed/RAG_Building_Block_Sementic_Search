...
class Word2VecRetriever:
    ...
    def __init__(self, vector_size: int = 100, window: int = 5, min_count: int = 1, 
                 sg: int = 0, epochs: int = 10, workers: int = 4):
        """
    ...
    def build_index(self, corpus_texts: List[str]) -> None:
        ...
        vectors_list = [self._text_to_vector(doc) for doc in corpus_texts]
        ...
        self.corpus_vectors = np.array(vectors_list)
    ...
    def retrieve(self, queries: List[str], k: int = 10) -> Dict[int, List[int]]:
        ...
        for q_idx, query in enumerate(queries):
            ...
            if query_vector is not None and not np.all(query_vector == 0):
    ...
    def optimize_parameters(param_combinations, corpus_texts, query_texts, qrels_dict, evaluator_class):
        ...
        best_config = None
        best_score = 0.0  # Initialize with appropriate starting value
        best_metrics = None
        results_log = []
        ...
        for i, params in enumerate(param_combinations, 1):
            ...
            try:
                ...
                with redirect_stderr(stderr_suppressor):
                    ...
                    mrr_score = test_metrics.get('mrr', test_metrics.get('MRR', 0.0))
                    recall_5 = test_metrics.get('recall@5', test_metrics.get('Recall@5', 0.0))
