...
class TransformerRetriever:
    ...
    def retrieve(self, query_texts: List[str], k: int = 20) -> Dict[int, List[int]]:
        ...
        query_embeddings = self.model.encode(query_texts, convert_to_tensor=True)

        search_results = util.semantic_search(query_embeddings, self.corpus_embeddings, top_k=k)
        
        # YOUR CODE HERE: Calculate similarities and retrieve top-k documents
        results = {}
        for query_idx, hits in enumerate(search_results):
            results[query_idx] = [hit['corpus_id'] for hit in hits]
